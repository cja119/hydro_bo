...
def run_planning(vector_type: str):
    environment = Planning(f"{vector_type}-Chile", weather_file="CoastalChile_15-20_Wind.csv")

    with environment as env:
        env["booleans"]["vector_choice"][vector_type] = True
        env["booleans"]["electrolysers"]["SOFC"] = True
        env["booleans"]["wind"] = True
        env["equipment"]["vector_production"][vector_type] = 4 if vector_type == "NH3" else 12
